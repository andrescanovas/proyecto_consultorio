...
miAltura = StringVar()
miCiudad = StringVar()
miProvincia = StringVar()
miPais = StringVar()
miMedicocabe = StringVar()
...
def borrar(tipo):
	...
	try:
		...
		if messagebox.askyesno(message="¿Realmente desea eliminar el registro?", title="ADVERTENCIA"):
			miCursor.execute("DELETE FROM "+tipo1+" WHERE ID="+miId.get())
			miConexion.commit()
	except:
		messagebox.showwarning("ADVERTENCIA","No se pudo eliminar")
		pass
	limpiarCampos()
	mostrar(tipo2)
...
l2.grid(pady=5,row=1, padx=10, column=1)
e2=Entry(root, textvariable=miNombre)
e2.grid(pady=5,row=1, column=2)
...
l8=Label(root, text="Calle")
l8.grid(row=2, column=5)
e8=Entry(root, textvariable=miCalle)
e8.grid(row=2, column=6) 

l9=Label(root, text="Altura")
l9.grid(row=2, column=7)
e9=Entry(root, textvariable=miAltura)
e9.grid(row=2, column=8) 

#-------------------------------
l10=Label(root, text="Ciudad")
l10.grid(row=3, padx=10,column=1)
e10=Entry(root, textvariable=miCiudad)
e10.grid(row=3, column=2) 

l11=Label(root, text="Provincia")
l11.grid(row=3, column=3)
e11=Entry(root, textvariable=miProvincia)
e11.grid(row=3, column=4) 

l12=Label(root, text="Pais")
l12.grid(row=3, column=5)
e12=Entry(root, textvariable=miPais)
e12.grid(row=3, column=6)

l13=Label(root, text="Medico Cabecera")
